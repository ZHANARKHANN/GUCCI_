...
allure {
    ...
    adapter {
        ...
        frameworks {
            junit5 { adapterVersion.set("2.29.0") }
        }
    }
}
...
/**
 * ЕДИНСТВЕННЫЙ верхнеуровневый таск test.
 * Поддерживает:
 *  -Dtags=SMOKE,WEB,Test_Case_5      (список тегов через запятую)
 *  -Djunit.jupiter.tags="SMOKE & WEB" (логическое выражение)
 */
tasks.test {
    useJUnitPlatform()

    // выражение (приоритетно): SMOKE & WEB | !SLOW
    val expr = System.getProperty("junit.jupiter.tags")?.trim()
    if (!expr.isNullOrEmpty()) {
        systemProperty("junit.jupiter.tags", expr)
        println(">> JUnit tags expression: $expr")
    } else {
        // список через запятую: SMOKE,WEB или Test_Case_5
...
/** (Опционально) отдельная задача для SMOKE */
tasks.register<Test>("smokeTest") {
    group = "verification"
    description = "Runs tests tagged with @Tag(\"SMOKE\")"
    useJUnitPlatform { includeTags("SMOKE") } // регистр важен!
}
